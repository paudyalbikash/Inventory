	...
	Datepicker.prototype = {
		...
		_process_options: function (opts) {
			...
			if (o.startDate !== -Infinity) {
				...
				o.startDate = this.getFullDate(2001, 1, 1);
			}
			if (o.endDate !== Infinity) {
				o.endDate = o.endDate;
			}
			else {
				o.endDate = this.getFullDate(2032, 12, 30);
			}
		...
		show: function () {
			...
			if (this.element.val()) {
				this.viewDate = this.element.val() || this.todayNpDate();
			}
			this.place();
			this.picker.show();
		...
		hide: function () {
			...
			if (this.inputField.val()) {
				this.viewDate = this.inputField.val();
				this.setValue();
			}
			this._trigger('hide');
			return this;
		...
		update: function () {
			...
			if (fromArgs) {
				...
				if (String(oldDate) !== String(this.dates) && fromArgs) {
					this._trigger('changeDate');
					this.element.change();
				}
			}
			if (!this.dates.length && oldDate) {
		...
		_fill_yearsView: function (selector, cssClass, factor, year, startYear, endYear, beforeFn) {
			...
			var focusedVal = Math.floor(this.viewDate.split('-')[0] / step) * step;
			var selected = Math.floor(parseInt(this.viewDate.split('-')[0]) / step) * step;
			...
			for (var currVal = startVal - step; currVal <= endVal + step; currVal += step) {
				...
				if (currVal < Number(startYear) || currVal > Number(endYear)) {
				...
				html += '<span class="' + classes.join(' ') + '"' + (tooltip ? ' title="' + tooltip + '"' : '') + 'data-target="' + currVal + '" >' + currVal + '</span>';
		...
		fill: function () {
			...
			this.picker.find('.datepicker-days .datepicker-switch')
				.text(DPGlobal.formatDate(this.viewDate, titleFormat, this.o.language));
		...
		click: function (e) {
			...
			//		//this.viewDate = this.getFullDate(oldView[0], oldView[1], 1);

			//		day = 1;
			//		if (this.viewMode === 1) {
			//			month = target.parent().find('span').index(target);
			//			let oldView = this.viewDate.split("-");
			//			//this.viewDate = this.getFullDate(oldView[0], month + 1, 1);
			//		} else {
			//			month = 1;
			//			year = Number($(target).attr("data-target"));
			//			//this.viewDate = this.getFullDate(year, month, 1);
			//		}
			//		this._trigger(DPGlobal.viewModes[this.viewMode - 1].e);
			...
			//			this.fill();
			//		}
			//		this.element.val(this.viewDate);
			//	}
			//}
		...
		dayCellClick: function (e) {
			...
			if (this.o.updateViewDate) {
				if (parseInt(timestamp.split("-")[0]) !== parseInt(this.viewDate.split("-")[0])) {
					this._trigger('changeYear', timestamp);
				}
				if (parseInt(timestamp.split("-")[1]) !== parseInt(this.viewDate.split("-")[1])) {
					this._trigger('changeMonth', timestamp);
				}
			}
			this._setDate(timestamp);
		},
		...
		navArrowsClick: function (e) {
			...
			var a = this.moveMonth(this.viewDate, dir);
			console.log(this.viewDate, a);
			this.viewDate = a;
